...
from datasets import EducationBBDataset, UCF10124Dataset
...
def train_classifier(
    ...
    for epoch in range(start_epoch, config.solver.epochs):
        ...
        for kkk,data in enumerate(tqdm(train_loader)):
            ...
            real_texts = []
            for i, j in zip(list_id, text_id):
                
                real_texts.append(text_aug_dict[j][i])
                # real_texts = [text_aug_dict[j][i] for i,j in zip(list_id,text_id)]
                # except:
                #     raise ValueError(list_id.shape, len(text_aug_dict.keys()), j, i, len(text_aug_dict[list(text_aug_dict.keys())[0]]))
            list_id = list_id.to(device)

            if not config.data.florence.activate:
                texts = token_texts.to(device)
            # if there are bounding box texts being usd lambda_enbb will be greater than 0
            elif config.data.weights.lambda_enbb > 0:
                with torch.no_grad():
                    # if ff_caption == None:
                    caption = False
                    if lambda_enff > 0:
            ...
            # if florence not active and lambda_enbb is 0 then that means just full frame texts
            else:
                with torch.no_grad():
                    # if ff_caption == None:
                    caption = False
            ...
            #     run_with_loss_type(lambda_enbb,
            #                        lambda_enff,
            #                        lambda_ff,
            #                        lambda_bb,
            #                        texts,
            ...
            kl_loss = config.loss.image*loss_imgs + config.loss.text*loss_texts 
...
def main():
    ...
    if 'education' in config.data.dataset:
        ...
        val_loader = DataLoader(
                        ...
                        drop_last=True,
                        collate_fn=collate_fn)
    elif 'ucf' in config.data.dataset:
        train_data = UCF10124Dataset('./ucf101_24/UCF101_v2', './ucf101_24/UCF101_v2/UCF101v2-GT.pkl', transforms=transform_train)
        train_loader = DataLoader(
                        train_data,
                        batch_size=config.data.batch_size,
                        num_workers=config.data.workers,
                        ...
                        drop_last=True, 
                        collate_fn=collate_fn)
        val_data = UCF10124Dataset('./ucf101_24/UCF101_v2', './ucf101_24/UCF101_v2/UCF101v2-GT.pkl', transforms=transform_train, split='test')
        val_loader = DataLoader(
                        val_data,
                        batch_size=config.data.batch_size,
                        num_workers=config.data.workers,
        ...
        raise TypeError(f'The dataset {config.data.dataset} is unknown to the SemanticBox model')
